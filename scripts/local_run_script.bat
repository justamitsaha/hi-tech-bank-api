...
if "%user_action%"=="y" (
    ...
    cd target/
    start java -jar config-service-1.0-SNAPSHOT.jar
    cd ../
) else (
...
if "%user_action%"=="y" (
    start mvn clean install
    cd target/
    start java -jar discovery-service-1.0-SNAPSHOT.jar >> output.txt
    cd ../
) else (
    echo "Skipping config server"
)
...
if "%user_action%"=="y" (
    start mvn clean install
    cd target/
    start java -jar api-gate-way-service-1.0-SNAPSHOT.jar
) else (
    echo "Skipping config server"
)
...
if "%user_action%"=="y" (
    start mvn clean install
    cd target/
    start java -jar redis-cache-service-1.0-SNAPSHOT.jar
    cd ../
) else (
    echo "Skipping config server"
)
...
if "%user_action%"=="y" (
    start mvn clean install
    cd target/
    start java -jar onboard-user-service-1.0-SNAPSHOT.jar
    cd ../
) else (
    echo "Skipping config server"
)